...
import {
    ...
    Divider,
    Text,
    Tabs, TabList, TabPanels, Tab, TabPanel,Container, Menu, MenuButton, MenuList, MenuItem, Image, Flex
    ...
    return (
      <>
        ...
        <Drawer placement={placement} onClose={onClose} isOpen={isOpen}>
          ...
          <Box p={"10px"}>{isOpen ? <CloseIcon onClick={onClose}/> : <HamburgerIcon /> }</Box>
          <Box><Image src='https://upload.wikimedia.org/wikipedia/commons/thumb/f/fd/Zara_Logo.svg/640px-Zara_Logo.svg.png' alt="Zara Logo"/></Box>       
          </Flex>
             <Tabs fontFamily={"Neue-Helvetica"} fontSize="16px">
                ...
                <TabPanels>
                    <TabPanel rowGap={"10"}>
                    <Box>New</Box>
                    <Link to={"/t-shirts"}><Box>T-shirts</Box></Link>
                    <Link to={"/trackSuits"}><Box>TrackSuits</Box></Link>
                    <Link to={"/trousers"}><Box>Trousers</Box></Link>
                    <Link to={"/sweatshirts"}><Box>SweatShirts</Box></Link>
                    </TabPanel>
                    <TabPanel>
                    <Box>New</Box>
                    <Link to={"/t-shirts"}><Box>T-shirts</Box></Link>
